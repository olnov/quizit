...
export async function createSoloRoom(
	...
	return request<CreateRoomResponse>('/api/v1/game-rooms/solo', {
		method: 'POST',
		body: JSON.stringify({
			quizId,
			hostName: playerName,
...
export async function getQuizzes(): Promise<PublicQuiz[]> {
	return request<PublicQuiz[]>('/api/v1/quizes', { method: 'GET' });
}
export async function getQuizDifficultyCounts(
	quizId: string
): Promise<Array<{ difficulty: number; count: number }>> {
	return request<Array<{ difficulty: number; count: number }>>(
		`/api/v1/quizes/${encodeURIComponent(quizId)}/difficulty-counts`,
		{ method: 'GET' }
	);
}

export async function getRoom(gameCode: string): Promise<GameRoom> {
	return request<GameRoom>(`/api/v1/game-rooms?gameCode=${encodeURIComponent(gameCode)}`, {
		method: 'GET'
	});
}

export async function startRoom(gameCode: string, playerToken: string): Promise<GameRoom> {
	return tokenRequest<GameRoom>(
		`/api/v1/game-rooms/${encodeURIComponent(gameCode)}/start`,
		playerToken
	);
}
export async function updateRoomSettings(
	...
	return request<GameRoom>(`/api/v1/game-rooms/${encodeURIComponent(gameCode)}/settings`, {
		method: 'POST',
		body: JSON.stringify({
			playerToken,
			questionCount,
...
export async function submitAnswer(
	...
	await request(`/api/v1/game-rooms/${encodeURIComponent(gameCode)}/answers`, {
		method: 'POST',
		body: JSON.stringify({ playerToken, answerOptionId })
	});
}
export async function nextQuestion(gameCode: string, playerToken: string): Promise<void> {
	await tokenRequest(`/api/v1/game-rooms/${encodeURIComponent(gameCode)}/next`, playerToken);
}
export async function completeGame(gameCode: string, playerToken: string): Promise<GameCompleted> {
	return tokenRequest<GameCompleted>(
		`/api/v1/game-rooms/${encodeURIComponent(gameCode)}/complete`,
		playerToken
	);
}
export async function restartRound(gameCode: string, playerToken: string): Promise<GameRoom> {
	return tokenRequest<GameRoom>(
		`/api/v1/game-rooms/${encodeURIComponent(gameCode)}/restart`,
		playerToken
	);
}
...
export async function connectToRoom(
	...
	const connection = new HubConnectionBuilder()
		.withUrl(`${await getBackendPublicUrl()}/api/v1/hubs/game`)
	...
	if (handlers.onScoreboardUpdated)
		connection.on('ScoreboardUpdated', handlers.onScoreboardUpdated);
...
export function saveGameResult(result: GameCompleted): void {
	sessionStorage.setItem(`quizit:result:${result.gameCode}`, JSON.stringify(result));
}
export function getGameResult(gameCode: string): GameCompleted | null {
	const value = sessionStorage.getItem(`quizit:result:${gameCode}`);
	if (!value) return null;
	try {
		return JSON.parse(value) as GameCompleted;
	} catch {
		return null;
	}
}

function tokenRequest<T>(path: string, playerToken: string): Promise<T> {
	return request<T>(path, { method: 'POST', body: JSON.stringify({ playerToken }) });
}
async function getBackendPublicUrl(): Promise<string> {
	backendPublicUrl ??= fetch('/api/runtime-config').then(async (response) => {
