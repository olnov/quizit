<script lang="ts">
	...
	let remainingSeconds = $derived(
		question?.answerDeadlineAt
			? Math.max(0, Math.ceil((Date.parse(question.answerDeadlineAt) - now) / 1000))
			: null
	);
	...
	onMount(() => {
		...
		void connectToRoom(
			...
			},
			{
				...
				onQuestionStarted: (updatedQuestion) => {
				...
				onGameCompleted: (completed) => {
					saveGameResult(completed);
					void goto(`/results/${params.gameCode}`);
				}
			}
		)
			...
			.catch((error) => {
				message = error instanceof Error ? error.message : 'Unable to join the game.';
			});
		return () => {
	...
	async function advance(action: 'next' | 'complete') {
		...
		try {
			...
			message = error instanceof Error ? error.message : 'Unable to update the game.';
		}
	}

	function formatTime(seconds: number | null) {
		return seconds === null ? 'Unlimited' : `00:${seconds.toString().padStart(2, '0')}`;
	}
</script>

<main class="game-shell">
	<header>
		<a class="brand" href="/"><span class="brand-mark">Q</span><span>QuizIt</span></a><span
			class="question-count"
			>Question {(question?.index ?? 0) + 1} / {room?.questionCount ?? '-'}</span
		>
	</header>
	{#if room && question}
		<section class="game-grid">
			<div class="question-panel">
				...
				{#if question.question.codeContext}
					<CodeContext code={question.question.codeContext} />
				{/if}
				...
				<div class="options">
					{#each question.question.options as option, index}
						<button
							...
							disabled={Boolean(selectedOptionId || reveal)}
							><b>{String.fromCharCode(65 + index)}</b>{option.text}</button
						>
					{/each}
				</div>
				{#if isHost}
