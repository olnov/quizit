<script lang="ts">
	...
	const selectedQuestion = $derived(
		quiz?.questions.find((question) => question.clientId === selectedQuestionId) ?? null
	);
	...
	function removeSelectedQuestion() {
		...
		const index = quiz.questions.findIndex(
			(question) => question.clientId === selectedQuestion.clientId
		);
	...
	async function saveQuiz() {
		...
		try {
			...
			selectedQuestionId =
				quiz.questions.find((question) => question.clientId === selectedQuestionId)?.clientId ??
...
<main class="admin-shell">
	...
	{:else if !quiz}
		<section class="failure">
			...
			<p>{error || 'The requested quiz could not be loaded.'}</p>
			<a href="/admin/quizes">Return to quizzes</a>
		</section>
	{:else}
		<section class="editor-header">
			<div>
				...
				/>
				<p class="save-state" class:unsaved={dirty}>
					{error || saveMessage || (dirty ? 'Unsaved changes' : 'All changes saved')}
				</p>
			</div>
			<div class="editor-actions">
				{#if quiz.status !== 1}<button type="button" class="text-action" onclick={publishQuiz}
						>Publish</button
					>{/if}
				{#if quiz.status !== 0}<button type="button" class="text-action" onclick={moveQuizToDraft}
						>Move to draft</button
					>{/if}
				{#if quiz.status !== 2}<button
						type="button"
						class="text-action"
						onclick={() => (confirmation = 'archive')}>Archive</button
					>{/if}
				<button type="button" class="text-action danger" onclick={() => (confirmation = 'delete')}
					>Delete</button
				>
				<Button type="button" disabled={saving || !dirty} onclick={saveQuiz}
		...
		<section class="quiz-settings" aria-label="Quiz settings">
			<label>
				<span>Theme</span>
				<output class="readonly-field">{quiz.themeName}</output>
			</label>
			<label
				><span>Questions per game</span><input
					bind:value={quiz.questionsPerGame}
					max="100"
					min="1"
					onchange={markDirty}
					type="number"
			...
			>
			<label
				><span>Question count mode</span><select
					bind:value={quiz.questionCountMode}
		...
		<div class="editor-layout">
			<aside class="question-list">
				<div class="list-title">
					<span>Questions ({quiz.questions.length})</span><button
						type="button"
						aria-label="Add question"
						onclick={addQuestion}>+</button
					>
				</div>
				{#if quiz.questions.length === 0}<p class="no-questions">
						Add the first question to start the quiz.
					</p>{/if}
				{#each quiz.questions as question, index (question.clientId)}
					<button
						class:active={selectedQuestionId === question.clientId}
						type="button"
						onclick={() => (selectedQuestionId = question.clientId)}
						><span>{String(index + 1).padStart(2, '0')}</span><strong
							>{question.text || 'Untitled question'}</strong
						><small>{question.difficulty}</small></button
					>
				{/each}
			</aside>
			...
			{#if selectedQuestion}
				...
				>
					<div class="question-topline">
						...
						<button class="delete-question" onclick={removeSelectedQuestion} type="button"
							>Remove question</button
						>
					</div>
					<label
						><span>Difficulty</span><input
							bind:value={selectedQuestion.difficulty}
							max="1000"
							min="0"
							oninput={markDirty}
							step="100"
					...
					<label
						><span>Question</span><textarea
							bind:value={selectedQuestion.text}
							maxlength="1000"
							oninput={markDirty}
							placeholder="Write the question players will see"
							rows="3"></textarea></label
					>
					<label
						><span>Code context</span><textarea
							bind:value={selectedQuestion.codeContext}
							class="code-field"
							maxlength="5000"
							oninput={markDirty}
							placeholder="Optional code or supporting context"
							rows="8"></textarea></label
					>
					<fieldset>
						<legend>Answer options</legend>
						<p class="field-hint">Select the correct answer, then write all four options.</p>
						<div class="options">
							{#each selectedQuestion.options as _, index}<label
									><input
										bind:group={selectedQuestion.correctOptionIndex}
										onchange={markDirty}
										name="correctOption"
										type="radio"
										value={index}
									/><span>{String.fromCharCode(65 + index)}</span><input
										bind:value={selectedQuestion.options[index]}
										maxlength="1000"
										oninput={markDirty}
					...
					<label
						><span>Explanation</span><textarea
							bind:value={selectedQuestion.explanation}
							maxlength="2000"
							oninput={markDirty}
							placeholder="Explain the correct answer after the reveal"
							rows="4"></textarea></label
					>
				</form>
			{:else}
				<section class="question-empty">
					<h2>No question selected</h2>
					<p>Add a question to start building the round.</p>
...
<AdminConfirmDialog
	...
	message={confirmation === 'delete'
		? 'This removes the quiz from the active library. Quizzes with game history cannot be deleted.'
		: confirmation === 'archive'
			? 'Archived quizzes cannot be selected for new games.'
			: 'Your latest edits will be lost.'}
	confirmLabel={confirmation === 'delete'
		? 'Delete quiz'
		: confirmation === 'archive'
			? 'Archive quiz'
			: 'Discard changes'}
...
<style>
	...
	.question-form > label:first-of-type {
		max-width: 160px;
	}
	.code-field {
	...
	@media (max-width: 560px) {
		...
		.editor-actions :global(.game-button) {
			width: 100%;
		}
	}
</style>
