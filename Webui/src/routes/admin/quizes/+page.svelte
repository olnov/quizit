<script lang="ts">
	...
	let search = $state('');
	let status = $state<QuizStatus | 'all'>('all');
	let themeId = $state('all');
	...
	async function loadQuizzes() {
		...
		error = '';
		try {
			const result = await getAdminQuizzes(page, pageSize, {
	...
	function changeTheme(event: Event) {
		themeId = (event.currentTarget as HTMLSelectElement).value;
		resetFilters();
	}
...
<main class="admin-shell">
	...
	<section class="controls" aria-label="Quiz filters">
		...
		>
		<label
			><span>Status</span><select value={status} onchange={changeStatus}
				><option value="all">All statuses</option><option value="0">Draft</option><option value="1"
					>Published</option
				><option value="2">Archived</option></select
			></label
		>
		<label
		...
		<section class="import-preview" aria-live="polite">
			<div>
				...
				{#if importValidation?.isValid && importValidation.preview}
					...
					<p>
						{importValidation.preview.theme} · {importValidation.preview.questionCount} questions · {importValidation
							.preview.questionsPerGame} per game
					</p>
				{:else}
	...
	<section class="quiz-list" aria-label="Quizzes">
		<div class="list-head">
			<span>Quiz</span><span>Theme</span><span>Status</span><span>Questions</span><span
				>Actions</span
			>
		</div>
		{#if loading}
			...
			<div class="empty-state">
				...
				<p>
					{search || status !== 'all' || themeId !== 'all'
						...
						: 'Create or import a quiz to start building a question set.'}
				</p>
			</div>
		{:else}
			{#each quizzes as quiz (quiz.id)}
				<article class="quiz-row">
					...
					<span
						class:published={quiz.status === 1}
						class:archived={quiz.status === 2}
						class="status-badge">{quizStatusLabel[quiz.status]}</span
					>
					<span
						>{quiz.questionCountMode === 1
							? `${quiz.questionCount} total`
	...
	<nav class="pagination" aria-label="Quiz pages">
		<Button
			type="button"
		...
		<span>Page {page} of {totalPages} · {totalCount} quizzes</span>
...
<style>
	...
	.controls {
		...
		display: grid;
		gap: 16px;
		grid-template-columns: minmax(0, 2fr) minmax(140px, 1fr) minmax(180px, 1fr) minmax(
				260px,
				1.35fr
			);
		padding: 16px 0;
